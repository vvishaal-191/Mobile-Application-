...
console.log('Preparing comprehensive updates for Manager Dashboard & Employee Status...');

// 1. Prepare syncManagerDashboard function string to inject into tpl-ManagerDashboard and index.html
const syncManagerDashboardFuncCode = `
    function syncManagerDashboard(targetDoc) {
      ...
      if (permBadge) {
        const notifCount = Math.max(store.MGR_NOTIFICATION_COUNT || 0, totalPending.length, pendingPerms.length);
      ...
      if (requestsList) {
        if (allRequests.length === 0) {
          ...
          allRequests.forEach(function(req) {
            ...
            const empName = req.employeeName || req.name || 'Priya Sharma';
            const empInitials = req.employeeInitials || req.initials || (empName.split(' ').map(function(w){return w[0];}).join('').toUpperCase().slice(0, 2)) || 'PS';
            ...
            const isEmpInactive = (status === 'approved' && !isPerm) || (typeof store.computeEmploymentStatus === 'function' && store.computeEmploymentStatus({ name: empName }) === 'Inactive');
...
// 2. Prepare CSS additions for .profile-status-pill active/inactive
const statusPillCss = `
    .profile-status-pill {
      padding: 6px 14px;
      border-radius: 999px;
    ...
    .profile-status-pill.active-status {
      background: #DCFCE7 !important;
      border: 1px solid #86EFAC !important;
      color: #15803D !important;
    }
    .profile-status-pill.active-status .profile-status-dot {
      background: #16A34A !important;
    }
    .profile-status-pill.active-status .profile-status-text {
      color: #15803D !important;
    }
    .profile-status-pill.inactive-status {
      background: #FEE2E2 !important;
      border: 1px solid #FCA5A5 !important;
      color: #DC2626 !important;
    }
    .profile-status-pill.inactive-status .profile-status-dot {
      background: #EF4444 !important;
    }
    .profile-status-pill.inactive-status .profile-status-text {
      color: #DC2626 !important;
    }
`;

// Export for use
fs.writeFileSync('scratch/sync_mgr_code.js', syncManagerDashboardFuncCode);
console.log('Saved scratch/sync_mgr_code.js');
